...
const colorThemes = {
  ...
  sunrise: { label: "Sunrise", chip: "from-orange-500/30 to-fuchsia-500/25", colors: { default: "bg-orange-400", comparing: "bg-fuchsia-400", swapping: "bg-red-500", sorted: "bg-lime-400", pivot: "bg-indigo-500", target: "bg-yellow-300" } },
...
export default function VisualizerPage({ name, cppSnippet, javaSnippet }) {
  ...
  return (
    <div className="font-body relative mx-auto w-full max-w-7xl px-4 py-8 sm:px-6 sm:py-10 lg:py-12">
      ...
      <motion.section initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} className="relative overflow-hidden rounded-3xl border border-white/10 bg-slate-800/40 p-5 shadow-2xl backdrop-blur sm:p-7">
        <div className="relative z-10 grid gap-6 xl:grid-cols-[1.15fr_0.85fr]">
          <div>
            <div className="mb-4 flex flex-wrap items-center gap-2">
              <span className="rounded-full border border-cyan-400/25 bg-cyan-500/10 px-3 py-1 text-xs font-semibold uppercase tracking-widest text-cyan-200">{algorithm?.category}</span>
              <span className={`rounded-full border px-3 py-1 text-xs font-semibold ${statusStyleMap[runStatus]}`}>{runStatus}</span>
            ...
            <h1 className="font-display text-3xl font-black text-white sm:text-5xl">{name}</h1>
            <p className="mt-3 text-sm text-slate-300">{algorithm?.description}</p>
            <div className="mt-5">
              ...
              <div className="h-2 overflow-hidden rounded-full bg-slate-700/70">
                <MotionDiv animate={{ width: `${progress}%` }} className={`h-full bg-gradient-to-r ${themeConfig.chip}`} />
              </div>
            </div>
            <div className="mt-5 grid grid-cols-2 gap-3 sm:grid-cols-4 text-center">
              {["Best", "Average", "Worst", "Space"].map((stat) => (
                <div key={stat} className="rounded-xl border border-white/10 bg-white/5 p-3">
                  <p className="text-[11px] uppercase tracking-wider text-slate-400">{stat}</p>
                  <p className="mt-1 text-sm font-semibold text-white">{algorithm?.[stat.toLowerCase()] || "-"}</p>
                </div>
              ))}
            </div>
          </div>
          <div className="rounded-2xl border border-white/10 bg-slate-900/55 p-5">
            <p className="inline-flex items-center gap-2 text-xs font-bold uppercase tracking-widest text-slate-300"><Radar size={14} className="text-cyan-300" /> Live Snapshot</p>
            <div className="mt-4 grid grid-cols-2 gap-3">
              <div className="rounded-xl bg-white/5 p-3"><p className="text-[11px] text-slate-400">Data Points</p><p className="text-lg font-bold text-white">{array.length}</p></div>
              <div className="rounded-xl bg-white/5 p-3"><p className="text-[11px] text-slate-400">Sorted</p><p className="text-lg font-bold text-emerald-200">{sortedCount}/{array.length}</p></div>
              <div className="rounded-xl bg-white/5 p-3"><p className="text-[11px] text-slate-400">Delay</p><p className="text-lg font-bold text-cyan-100">{speed}ms</p></div>
              <div className="rounded-xl bg-white/5 p-3"><p className="text-[11px] text-slate-400">Avg Value</p><p className="text-lg font-bold text-blue-100">{valueStats.avg}</p></div>
            </div>
          </div>
        </div>
      </motion.section>
      ...
      <div className="mt-6 grid grid-cols-1 gap-6 xl:grid-cols-[360px_1fr]">
        <aside className="rounded-3xl border border-white/10 bg-slate-800/35 p-5 backdrop-blur xl:sticky xl:top-24">
          <div className="mb-5 flex items-center gap-2"><SlidersHorizontal size={18} className="text-cyan-300" /><h2 className="text-sm font-bold uppercase tracking-widest text-white">Controls</h2></div>
          <div className="space-y-4">
            <div className="rounded-2xl bg-white/5 p-3">
              <label className="flex justify-between text-xs text-slate-400 mb-2 uppercase"><span>Size</span> <span>{arraySize}</span></label>
              <input type="range" min="16" max="80" value={arraySize} disabled={isSorting} onChange={(e) => { setArraySize(+e.target.value); handleGenerateNew(+e.target.value); }} className="w-full accent-cyan-400" />
            </div>
            <div className="rounded-2xl bg-white/5 p-3">
              <label className="flex justify-between text-xs text-slate-400 mb-2 uppercase"><span>Delay</span> <span>{speed}ms</span></label>
              <input type="range" min="10" max="150" value={speed} disabled={isSorting} onChange={(e) => setSpeed(+e.target.value)} className="w-full accent-blue-400" />
            </div>
            <div className="grid grid-cols-2 gap-2">
              <MotionButton onClick={handleResetHighlights} className="flex items-center justify-center gap-2 rounded-xl bg-white/5 py-2.5 text-sm font-bold text-white border border-white/10"><RotateCcw size={16} /> Reset</MotionButton>
              <MotionButton onClick={() => handleGenerateNew()} className="flex items-center justify-center gap-2 rounded-xl bg-cyan-500/10 py-2.5 text-sm font-bold text-cyan-100 border border-cyan-400/20"><Shuffle size={16} /> New Data</MotionButton>
            </div>
            <div className="grid grid-cols-2 gap-2 items-start">
              <div className="flex flex-col">
                <MotionButton onClick={() => !isTooLargeForValues && setShowValues(!showValues)} className={`flex items-center justify-center gap-2 rounded-xl py-2.5 text-sm font-bold border transition-all ${isTooLargeForValues ? "bg-slate-800/50 border-white/5 text-slate-500 cursor-not-allowed" : "bg-white/5 border-white/10 text-white hover:bg-white/10"}`}>
                  {showValues && !isTooLargeForValues ? <EyeOff size={16} /> : <Eye size={16} />} {isTooLargeForValues ? "Hidden" : (showValues ? "Hide" : "Values")}
                </MotionButton>
                {isTooLargeForValues && <p className="mt-1 text-[9px] text-amber-400/90 text-center font-medium animate-pulse">Size must be ≤ 35</p>}
              </div>
